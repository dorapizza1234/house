...
| 가족 공용 식물 | 프리셋 메시지 | 회원가입 (SMS 인증) |
|:---:|:---:|:---:|
| ![plant](docs/screenshots/plant.png) | ![message](docs/screenshots/message.png) | ![signup](docs/screenshots/signup.png) |
...
- **가비아 도메인** — [dorapizza.store](https://dorapizza.store) (A 레코드 → Vercel)
...
환경변수 필요:
- `DB_PASSWORD` (Supabase Postgres)
- `JWT_SECRET`
- `COOLSMS_API_KEY` / `COOLSMS_API_SECRET` / `COOLSMS_FROM_NUMBER`
- `MAIL_USERNAME` / `MAIL_PASSWORD` (Gmail 앱 비밀번호 16자리)
- Redis: 로컬 Docker `redis:7-alpine`
...
기본 포트: 백엔드 `8080`, 프론트 `5173` (CORS 설정됨)
...
- [x] **Railway (백) + Vercel (프론트) 배포 완료**
- [x] **가비아 DNS A 레코드 → Vercel 전환 완료** ([dorapizza.store](https://dorapizza.store))
